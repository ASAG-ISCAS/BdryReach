...
void _f(Node/* t*/, Node in[], int /*dimIn*/, Node out[], int/* dimOut*/, Node params[], int noParams){
    out[0] = -in[1] - 1.5*in[0]*in[0] - 0.5*in[0]*in[0]*in[0] - 0.5;
    out[1] = 3*in[0] - in[1];
...
int main()
{
    ...
    center << 1, 1;
    Matrix_t<double> generators(Eigen::MatrixXd::Identity(2,2) * 0.1);
	...
	vector<Zonotope<double>> underR = UnderApprox::underReachClp(mysys, mysysBack, options, R0_, 4, 1, 0.01, 0.01,0.02, 50, 20);
