...
typename MatrixType::ScalarType det(MatrixType A)
{
  ...
  for(j=0;j<dimension-1;++j)
  {
    ...
    Real a_max = rightBound(capd::abs(A[p[j]][j]));
    size_type i_max = j;
    for(i=j+1;i<dimension;++i){
      Real a_max_temp = rightBound(capd::abs(A[p[i]][j]));
      if( a_max_temp > a_max) {
        a_max = a_max_temp;
        i_max = i;
      }
...
void gauss(MatrixType a, ResultType b, ResultType& result)
{
  ...
  for(j=0;j<dimension-1;++j)
  {
    ...
    a_max = rightBound(capd::abs(a[p[j]][j]));
    i_max = j;
    temp = p[j];
    for(i=j+1;i<dimension;++i)
    {
      if( (a_max_temp = rightBound(capd::abs(a[p[i]][j]))) > a_max)
      {
        a_max = a_max_temp;
        i_max = i;
      }
...
//    return 1;
  ...
  while( !(s<d*diagonalizingRelTolerance) )
  {
   // std::cout << "\n ====\n D = " << D;
    ...
    if( isSingular(dominant) ) break;  //proceeding with iteration can only worsen things
...
//      return 0;
    ...
    if( isSingular(s) ) break;  //proceeding with iteration can only worsen things
  }
 // std::cout << "\n ====\n D = " << D << "\n===\n s = " << s << "\n d = " << d <<"\n";
  return 1;
}
