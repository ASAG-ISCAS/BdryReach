...
public:
  ...
  Interval & operator /= ( const Interval & A_iv ){
    if(! A_iv.contains(capd::TypeTraits<BoundType>::zero())){
      m_interval /= A_iv.m_interval;
      return *this;
    } else {
      throw typename Interval::IntervalError("Division by 0 in operator/=(Interval, Interval)", A_iv.inf(),  A_iv.sup());
    }
...
//==== declarations in intervalFriend.h  definitions in intervalOp.hpp ==================
  ...
  friend inline Interval operator - (const Interval& A_iv){
    return Interval(-A_iv.m_interval);
  }
  friend inline Interval operator + (const Interval& A_iv1, const Interval& A_iv2) {
  ...
  /// DW: this template is defined to prevent implicit conversion of exponent type
  /// In the previous implementation interval^double was converted to
  /// interval^int
  template<class E>
  friend inline Interval operator^(const Interval& x, E){
    std::ostringstream out;
    out << " power(interval x, E n): exponent type typeid(E).name()=" << typeid(E).name() << " is not supported. \n";
    throw typename Interval::IntervalError(out.str().c_str(), x.leftBound(), x.rightBound());
  }

  friend inline Interval operator ^ (const Interval & x, unsigned int n){
    if(n<=INT_MAX)
      return Interval(power(x.m_interval, (int)n));
    int m = (int)(n << 1);
    return (n&1) ? x*sqr(power(x,m)) : sqr(power(x,m));
  }

  friend inline Interval operator ^ (const Interval & x, int n){
    if( n>=0 or !isSingular(x) )
      return Interval(power(x.m_interval, n));
    throw typename Interval::IntervalError(" power(interval x, negativeInt): Interval x must be nonzero. \n", x.leftBound(), x.rightBound());
  }

  friend inline Interval operator ^ (const Interval & x, Interval::BoundType b){
    if( x.leftBound()>0.0 )
      return Interval(pow(x.m_interval, Interval::BaseInterval(b)));
    throw typename Interval::IntervalError(" power(interval x, double): Interval x must be positive. \n", x.leftBound(), x.rightBound());
  }
  ...
  friend inline Interval operator ^ (const Interval & a, const Interval & b){
    ...
    throw typename Interval::IntervalError(" power(interval A, interval B): Interval A must be positive. \n", a.leftBound(), a.rightBound());
  }
...
//     A_diam = ( l > r) ? l : r;
  ...
  template<class E>
  friend inline Interval power(const Interval& x, E n){
    return x^n;
  }
...
T width(const Interval<T, R, M> & ix){
    return ix.rightBound() - ix.leftBound();
}
