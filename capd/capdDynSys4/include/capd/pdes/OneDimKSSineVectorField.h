...
OneDimKSSineVectorField::ScalarType OneDimKSSineVectorField::dyyLogarithmicNorm(const VectorType& a, const size_type m) const{
  ...
  // is bounded by 2*k*L1Norm(a). Thus we have to find an upper bound of the quantity
  // h(k) = k^2(1-nu*k^2) + 2k*a_{2k} <= k^2(1-nu*k^2) + 2k*N
  // This function takes extremum if h'(k) = -4nu*k^3 + 2k + 2N =0
  ...
  for(int i=0;i<M;++i)
    N += abs(a[i]);
  int k0 = 1 + ceil(capd::max(0.5*N/nu,0.5/nu).rightBound());
  k0 = capd::max(k0,(int)m+1);

  // The lower bound for the result is the diagonal part at k0-row
  int k2 = k0*k0;
  ScalarType result = (k2*(1.-nu*k2)+2.*k0*N).rightBound();
  for(int k=m+1;k<k0;k++)
  {
    int k2 = k*k;
    ScalarType result = capd::max((k2*(1.-nu*k2)+2.*k*N).rightBound(),result.rightBound());
  }
