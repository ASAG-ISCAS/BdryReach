...
docker -H unix:///run/docker.sock run --network vnet --rm --cidfile="${cid_file}" --user="$(id -u)" --workdir="$PWD" --volumes-from="$(hostname -s)" $volumes -e "TARGET_HOST=${TARGET_HOST}"  capd/"${docker_image}" /bin/bash -c "${cmd}"
